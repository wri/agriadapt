...
export default function User(state = initialState, action) {
  switch (action.type) {
    ...
    case SET_USER_AREA_LAYER_GROUP: {
      const layerGroup = {
        [action.payload.area.id]: action.payload.layerGroups,
      };
    ...
    (dispatch) => {
      ...
      if (geostore) {
        return fetchGeostore(geostore).then((geo) => {
          dispatch(
            setUserAreaLayerGroup({ area, layerGroups: [setGeoLayer(geo)] })
          );
        });
      }
      ...
      return fetchCountry(iso.country).then((country) => {
        dispatch(
          setUserAreaLayerGroup({
            area,
            layerGroups: [setCountryLayer(country)],
          })
        );
      });
    }
...
export const getUserAreas = createThunkAction(
  ...
  () => (dispatch, getState) => {
    ...
    axios
      .all([fetchUserAreas(token), fetchSubscriptions(token)])
      .then(
        axios.spread((userAreas, subscriptions = []) => {
          const datasetsToFetch = new Set();
          subscriptions.forEach((_subscription) => {
            (_subscription.datasets || []).forEach((_dataset) =>
              datasetsToFetch.add(_dataset)
            );
          });
  ...
  (area = {}) =>
    (dispatch, getState) => {
      ...
      if (area.subscriptions) {
        return axios
          .all(
            area.subscriptions.map((_sub) =>
              deleteSubscription(_sub.id, user.token)
            )
          )
          .then(() =>
