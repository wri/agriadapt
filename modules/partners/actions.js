...
// services
import { fetchPartners, fetchPartner } from "services/partners";
...
export const getAllPartners = createThunkAction(
...
export const getPartner = createThunkAction(
  "PARTNERS/GET-PARTNER",
  (id) => (dispatch) => {
    if (!id)
      throw new Error("A partner ID is mandatory to perform this action.");
...
export const getDatasetsByPartner = createThunkAction(
  "PARTNERS/GET-PARTNER",
  (datasetIds = []) =>
    (dispatch, getState) => {
      ...
      return fetchDatasets({
        ...
        .catch((err) => {
          dispatch(setError({ key: "datasetsByPartner", value: err.message }));
        });
    }
);
