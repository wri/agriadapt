...
export const getLatestPosts = createThunkAction(
  ...
  () => (dispatch) => {
    ...
    return fetchPosts({
      ...
      .then((posts) => {
        dispatch(setLatestPosts(postParser(posts)));
      })
      .catch(() => {
        dispatch(setLatestPostsError(ERROR_MESSAGE_FETCH_POSTS));
      });
  }
);
...
export const getSpotlightPosts = createThunkAction(
  ...
  () => (dispatch) => {
    ...
    return fetchPosts({
      ...
      .catch(() => {
        dispatch(setSpotlightPostsError(ERROR_MESSAGE_FETCH_POSTS));
      });
  }
);
