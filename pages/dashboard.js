...
export default function Dashboard(props) {
    ...
    useEffect(() => {
        ...
        map.current.on('load', () => {
            const { slug, layerUrl } = datasets[0];
            addRasterTileLayerToMap(map.current, slug, layerUrl);
            const { source } = datasets[1];
...
export async function getServerSideProps(context) {
    ...
    const resps = await Promise.allSettled(
        uuids.map((u) => {
            return fetchDataset(u);
        }));

    const datasets = resps.map(d => d.value);

    return { props: { ...config, datasets } };
}

const fetchDataset = async (uuid) => {
    return axios.get(`https://api.resourcewatch.org/v1/dataset/${uuid}/?includes=layer,metadata`)
        .then(({ data }) => {
