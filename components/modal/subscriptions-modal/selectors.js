...
const parseUserAreas = createSelector([getUserAreas], (_userAreas) =>
  _userAreas.map((userArea) => ({
    label: userArea.name,
    value: userArea.id,
...
export const isAreaFound = createSelector(
  ...
  (_userSubscriptions, _userSelection) => {
    ...
    return !!_userSubscriptions.find(
      (subscription) => subscription.params.area === areaID
    );
  }
);
