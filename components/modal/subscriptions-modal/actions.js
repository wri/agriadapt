    ...
    axios
      ...
      .then(
        axios.spread((...responses) => {
          dispatch(
            setAlertsPreview(responses.map((response) => response.data.data))
          );
        })
      )
      .catch((err) => {
    ...
    fetchDatasets({
      ...
      .then((datasets = []) => {
        dispatch(
          setDatasets(
            datasets.filter(
              (dataset) => Object.keys(dataset.subscribable).length
            )
          )
        );
...
export const updateSubscription = createThunkAction(
  ...
  () => (dispatch, getState) => {
    ...
    // Added datasets
    const addedDatasetsIds = newDatasetsIds.filter(
      (dId) => !oldDatasetsIds.find((e) => e === dId)
    );
    const addedDatasets = datasets.filter(
      (d) => !!addedDatasetsIds.find((e) => d.id === e)
    );
    // Datasets kept
    const datasetsKeptIds = oldDatasetsIds.filter((dId) =>
      newDatasetsIds.find((e) => e === dId)
    );
    const datasetsKept = oldDatasets.filter((d) =>
      datasetsKeptIds.find((e) => d.datasetId === e)
    );
