...
class GlobeCesiumComponent extends PureComponent {
  ...
  componentDidMount() {
    ...
    this.viewModel = {
      ...
      raise(layer, index) {
        ...
        this.viewModel.downLayer =
          this.viewModel.layers[Math.max(0, index - 1)];
        this.updateLayers();
        window.setTimeout(() => {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    // ---------- initialPosition ----------
    if (
      this.props.globeCesium.initialPosition.latitude !==
    ...
    // ---------- position ---------------
    if (
      this.props.globeCesium.position.latitude !==
        nextProps.globeCesium.position.latitude ||
      this.props.globeCesium.position.longitude !==
        nextProps.globeCesium.position.longitude ||
      this.props.globeCesium.position.height !==
        nextProps.globeCesium.position.height
    ) {
  ...
  getShapes(props) {
    ...
    if (layerPoints) {
      shapes = compact(
        layerPoints.map((elem) => {
          if (!layerActive.interactionConfig) {
            return null;
          }

          const tooltipContentObj = layerActive.interactionConfig.output.map(
            (obj) => ({
              key: obj.property,
              value: elem[obj.column],
              type: obj.type,
            })
          );
          ...
          if (elem.mag) {
            ...
            height =
              elem.distance_km > 0
                ? Math.log(elem.distance_km) * 30000
                : defaultHeight;
          } else if (elem.fatalities) {
            height =
              elem.fatalities > 0 ? elem.fatalities * 100000 : defaultHeight;
          }
  ...
  updateLayers(props, updateBasemap = true, updateLayers = true) {
    const {
      basemap,
      activeContextLayers,
      contextLayersOnTop,
    ...
    if (!contextLayersOnTop && updateLayers) {
      ...
      activeContextLayers.forEach((l) =>
        this.addAdditionalLayerOption(
          l.id,
          new Cesium.UrlTemplateImageryProvider({ url: l.url }),
          1,
          true
        )
      );
  ...
  render() {
    return <div id="cesiumContainer" className="c-globe-cesium" />;
  }
}
