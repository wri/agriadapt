...
class LayerPopup extends PureComponent {
  ...
  componentDidMount() {
    ...
    const layer =
      layers.find((_layer) => _layer.id === layersInteractionSelected) ||
      layers[0];
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    const layer =
      layers.find((_layer) => _layer.id === nextLayersInteractionSelected) ||
      layers[0];
  ...
  render() {
    ...
    const layer =
      layers.find((l) => l.id === layersInteractionSelected) || layers[0];
      ...
      <div className="c-layer-popup">
        <header className="popup-header">
          <select
            ...
            value={layer.id}
            onChange={(e) =>
              this.props.onChangeInteractiveLayer(e.target.value)
            }
          >
            {layers.map((o) => (
