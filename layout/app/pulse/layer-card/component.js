...
class LayerCardComponent extends PureComponent {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (
      (nextProps.layerMenuPulse.layerActive &&
        nextProps.layerMenuPulse.layerActive.id) !==
      (this.props.layerMenuPulse.layerActive &&
        this.props.layerMenuPulse.layerActive.id)
    ) {
      this.loadWidgets(nextProps);
      this.props.loadDatasetData(
        nextProps.layerMenuPulse.layerActive
  ...
  loadWidgets(nextProps) {
    ...
    const layerActive =
      layerMenuPulse.layerActive && layerMenuPulse.layerActive.id;

    if (layerActive) {
      let found = false;
      for (let i = 0; i < LAYERS_PLANET_PULSE.length && !found; i++) {
        found = LAYERS_PLANET_PULSE[i].layers.find(
          (obj) => obj.id === layerActive
        );
      }
      if (found) {
        const { widgets } = found;
  ...
  render() {
    const {
      showSubscribeToDatasetModal,
      showInfoModal,
      showContextLayersInfoModal,
    } = this.state;
    const { layerMenuPulse, layerCardPulse, activeContextLayers, router } =
      this.props;
    ...
    return (
      <div className={className}>
        <div>
          ...
          {source && <div className="source-container">{source}</div>}
            ...
            {layerName && (
              <div className="layer-container">
                ...
                >
                  ...
                  >
                    <LayerInfoModal layer={layerActive} />
                  </Modal>
                </button>
              </div>
            )}
                  ...
                  <div key={ctLayer.name}>
                    <div className="layer-container">
                      ...
                      <button
                        ...
                        aria-label="More information"
                        onClick={() =>
                          this.setState({ showContextLayersInfoModal: true })
                        }
                      >
                        <Icon name="icon-info" />
                        ...
                        <Modal
                          ...
                          className="-medium"
                          onRequestClose={() =>
                            this.setState({ showContextLayersInfoModal: false })
                          }
                        >
                          <LayerInfoModal layer={ctLayer} />
                        </Modal>
                      </button>
                    </div>
                    <Legend
