...
export const isATimelineDataset = createSelector(
  [getLayerGroup],
  (_layerGroup) =>
    !!_layerGroup && _layerGroup.layers.find((l) => l.layerConfig.timeline)
);
...
export const getTimelineLayer = createSelector(
  ...
  (_layerGroup, _isATimeline, _layers) => {
    if (_isATimeline) {
      const activeLayerValue =
        _layerGroup && _layerGroup.layers.find((l) => l.active);
      // We need to use the layer from the "layers" property, instead of the layer object
      // found in the Redux state
      const activeLayer =
        activeLayerValue && _layers.find((l) => l.id === activeLayerValue.id);
